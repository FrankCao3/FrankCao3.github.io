...
在给id赋值时加上其他无意义的字符`id=1abc`或者`id=1 and 1=1`、`id=1 and 1=2`、 `1' and 1=1#`、 `1' and 1=2#`之类的字符串，如果网站能正常回显页面（通过或者报错），证明有SQL注入漏洞。如果网站显示404或者500之类的错误，说明网站进行了字符过滤之类的操作，没有SQL注入漏洞。
...
- **into outfile** 或 **into dumpfile** ：写入文件（写入后门、木马）
